...
class user extends database{
	...
	function check($id, $tanggal) {
		$query = "SELECT count(*) as total FROM tbl_pesan where id_paket = $id and tgl_tour = '$tanggal'";
		$result = mysqli_query($this->getConnection(), $query);
		$hasil = mysqli_fetch_array($result);

		if((int)$hasil['total'] >= 2) {
			return false;
		} else {
			return true;
		}
...
public function order($id_user,$id_paket,$id_daerah,$tgl_pesan,$tgl_tour){
	
			$sql = "INSERT INTO `tbl_pesan` (`id_pesan`, `id_user`, `id_paket`, `id_daerah`, `tgl_pesan`, `tgl_tour`) VALUES (NULL, '$id_user','$id_paket','$id_daerah','$tgl_pesan','$tgl_tour')";
				$result = mysqli_query($this->getConnection(),$sql);
